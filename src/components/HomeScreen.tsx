...
const HomeScreen: React.FC<{
  ...
  avatarUrl?: string;
  onOpenSettings?: () => void;
  creditScore?: number;            // ⭐ MỚI - Điểm tín dụng
  ...
  const handleAvatarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setAvatar(url);
    e.target.value = ""; // reset input để chọn lại cùng file vẫn preview được
  };
  ...
  // Auto-sync from props (App.tsx already handles localStorage polling)
  const [syncedVipLevel, setSyncedVipLevel] = useState(vipLevel);
  ...
  // VIP rates mapping - 9 cấp độ (VIP0 - VIP8)
  const vipRates: { [key: string]: { discount: number; commission: number } } = {
    "VIP0": { discount: 0.05, commission: 0.010 },  // 5% discount, 1% commission
  ...
  const getCurrentVipInfo = () => {
    const vipKey = syncedVipLevel.replace(/\s+/g, '');
    const rate = vipRates[vipKey] || vipRates["VIP1"];
    return {
      commission: `${(rate.commission * 100).toFixed(1)}%`,
      discount: `${(rate.discount * 100).toFixed(0)}%`
    };
  };

  const currentVipInfo = getCurrentVipInfo();

  // 9 VIP levels với thông tin chi tiết
  const levels = [
    { level: "VIP 0", start: 0, discount: "1%", maxOrder: 3, color: "from-gray-300 to-gray-400", commission: "1.0%" },
    { level: "VIP 1", start: 100, discount: "1.5%", maxOrder: 5, color: "from-slate-200 to-slate-300", commission: "1.5%" },
    { level: "VIP 2", start: 388, discount: "2%", maxOrder: 8, color: "from-sky-200 to-sky-400", commission: "2.0%" },
    { level: "VIP 3", start: 1888, discount: "2.5%", maxOrder: 15, color: "from-blue-300 to-blue-500", commission: "2.5%" },
    { level: "VIP 4", start: 3888, discount: "3%", maxOrder: 20, color: "from-indigo-300 to-indigo-500", commission: "3.0%" },
    { level: "VIP 5", start: 8888, discount: "3.5%", maxOrder: 25, color: "from-purple-300 to-purple-500", commission: "3.5%" },
    { level: "VIP 6", start: 12888, discount: "4%", maxOrder: 30, color: "from-orange-300 to-orange-500", commission: "4.0%" },
    { level: "VIP 7", start: 48888, discount: "4.5%", maxOrder: 35, color: "from-amber-300 to-amber-500", commission: "4.5%" },
    { level: "VIP 8", start: 88888, discount: "5%", maxOrder: 40, color: "from-yellow-300 to-yellow-500", commission: "5.0%" },
  ];

  const handleCreateDepositOrder = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      console.log('📤 Đang gửi yêu cầu nạp tiền lên Supabase...');
      
      // GỬI YÊU CẦU NẠP TIỀN LÊN SUPABASE
      const result = await createDepositRequest({
        uid: userData.uid || userData.id || 0,
        username: userData.username || accountName || "unknown",
        amount: amountNumber,
        method: "Chuyển khoản ngân hàng"
      });

      if (result.error) {
        console.error('❌ Lỗi khi gửi yêu cầu nạp tiền:', result.error);
        alert(`❌ Lỗi: ${result.error}\n\nVui lòng thử lại hoặc liên hệ CSKH.`);
        return;
      }

      console.log('✅ Yêu cầu nạp tiền đã được gửi:', result);
      ...
      console.error('❌ Exception khi tạo deposit request:', error);
  ...
  const handleWithdrawAmountConfirm = () => {
    ...
    if (amount > syncedBalance) {
      alert(`❌ Số dư không đủ!\n\n💰 Số dư khả dụng: ${formatCurrency(syncedBalance)}\n💸 Số tiền muốn rút: ${formatCurrency(amount)}`);
      return;
    }
    setWithdrawStep(2);
  };
  ...
  const handleWithdrawFinalConfirm = async () => {
    ...
    try {
      console.log('📤 Đang gửi yêu cầu rút tiền lên Supabase...');
      
      // GỬI YÊU CẦU RÚT TIỀN LÊN SUPABASE
      const result = await createWithdrawRequest({
        uid: currentUser.uid || currentUser.id || 0,
        username: currentUser.username || accountName || "unknown",
        amount: amount,
      ...
      if (result.error) {
        console.error('❌ Lỗi khi gửi yêu cầu rút tiền:', result.error);
        alert(`❌ Lỗi: ${result.error}\n\nVui lòng thử lại hoặc liên hệ CSKH.`);
        return;
      }

      console.log('✅ Yêu cầu rút tiền đã được gửi:', result);
      ...
      console.error('❌ Exception khi tạo withdraw request:', error);
      alert("❌ Có lỗi xảy ra! Vui lòng thử lại.");
    }
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto pb-20">
      ...
      {/* VIP Card Display - Auto-sync from backend */}
      <section className="px-4 mb-4">
        <div className="relative">
          {/* Account Name Overlay on VIP Card */}
          <div className="absolute top-4 left-4 z-10">
            <p className="text-[10px] font-medium text-slate-300/80 mb-0.5 tracking-wide">ACCOUNT</p>
            <p className="text-sm font-bold bg-gradient-to-r from-slate-200 via-slate-100 to-slate-300 bg-clip-text text-transparent drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
              {accountName}
            </p>
          </div>
          
          <img 
            src={`/banners/logo-vip${/VIP\s*(\d+)/i.test(syncedVipLevel) ? syncedVipLevel.match(/VIP\s*(\d+)/i)?.[1] : "1"}.jpg.png`}
            alt={`${syncedVipLevel} Card`}
          ...
          />
          {/* VIP Info Overlay */}
          <div className="mt-3 bg-gradient-to-r from-slate-800/95 to-slate-900/95 backdrop-blur-sm rounded-xl p-4 border border-slate-700/50">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-slate-400 text-xs mb-1">Tài khoản VIP</p>
                <p className="text-white font-bold text-lg">{syncedVipLevel}</p>
              </div>
              <div className="text-right">
                <p className="text-slate-400 text-xs mb-1">Điểm tín dụng</p>
                <p className="text-amber-400 font-bold text-lg">⭐ {creditScore}</p>
              </div>
            </div>
            <div className="mt-3 pt-3 border-t border-slate-700/50 grid grid-cols-2 gap-3">
              <div>
                <p className="text-slate-400 text-xs">Hoa hồng/đơn</p>
                <p className="text-green-400 font-semibold text-sm mt-1">{currentVipInfo.commission}</p>
              </div>
              <div className="text-right">
                <p className="text-slate-400 text-xs mb-1">Số dư khả dụng</p>
                <p className="text-white font-semibold text-sm mt-1">{formatCurrency(syncedBalance)}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="px-4 mt-4">
        ...
        <div className="overflow-x-auto pb-2">
          <div className="flex gap-3 items-center px-1">
            {levels.map((vip, idx) => {
              // Xác định cấp hiện tại dựa trên syncedVipLevel
              const currentVipIdx = (() => {
                const m = syncedVipLevel.match(/VIP\s*(\d+)/i);
              ...
              return (
                <div
                  ...
                  <img src={`/banners/logo-vip${idx}.jpg.png`} alt={`VIP ${idx}`} className="absolute inset-0 w-full h-full object-cover" />
                  <div className={`relative p-3 text-white`}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm font-semibold">{vip.level}</span>
                      {idx === currentVipIdx && <span className="text-[9px] px-2 py-0.5 rounded-full bg-black/30 text-white font-semibold">Hiện tại</span>}
                    </div>
                    <div className="text-[11px]">
                      <div className="truncate">Hoa hồng <span className="font-semibold">{vip.commission}</span></div>
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="w-full max-w-md rounded-2xl bg-gradient-to-b from-slate-700 to-slate-800 text-white p-4 shadow-2xl my-auto">
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
              {levels.map((vip, index) => {
                ...
                const isCurrent = index === currentVipIdx;
      ...
      {/* Danh sách đơn hàng chạy liên tục */}
            ...
            <div className="scroll-orders-container">
              ...
              {[...Array(2)].map((_, duplicateIndex) => (
                <div key={duplicateIndex}>
                  <div className="bg-slate-900/40 border border-slate-700/50 rounded-lg mx-4 mb-2 p-2">
              ...
              <>
                <div className="mb-3">
                  ...
                  <div className="flex justify-between items-center mt-1">
                    ...
                    <p className="text-[10px] text-green-600 font-medium">Khả dụng: {formatCurrency(syncedBalance)}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-1.5 mb-3">
                  ...
                  <button
                    ...
                    disabled={balance <= 0}
                    className={`py-1.5 rounded-lg ${balance > 0 ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-400 cursor-not-allowed"} text-[11px] font-medium`}
                  >
                    Tối đa
                  </button>
                </div>
      ...
      <section className="px-4 pb-8 pt-4">
        <div className="text-center">
          ...
          <div className="flex items-center justify-center gap-4 mb-3">
            {/* Facebook */}
            <a 
              href="https://www.facebook.com/SCLM.FlowPay.vn/" 
              target="_blank" 
            ...
            {/* TikTok */}
            <a 
              href="https://www.tiktok.com/@sclm.vn" 
              target="_blank" 
              rel="noopener noreferrer"
            ...
            {/* Instagram */}
            <a 
              href="https://www.instagram.com/sclm.vn/" 
              target="_blank" 
              rel="noopener noreferrer"
